...
# find volume name in yaml file
    osd_nodes_number = parsed["osd_node"]["number"]
    volumes_to_create = parsed["osd_node"]["osds_number"]
...
# if they exists we are going to delete them and create new ones
    ...
    for number in range(1, volumes_to_create + 1):
        volume_name = "vgr" + str(number) + "_volume"
        for volume in range(1, osd_nodes_number + 1):
            if volume_name in openstack_volume_list.split():
                print "Deleting volume " + volume_name + str(volume)
                subprocess.call("openstack volume delete " + volume_name + str(volume), shell=True)
                if opt not in "--delete-volumes":
                    print "Creating volume " + volume_name + str(volume)
                    subprocess.call("openstack volume create --size " + str(volume_size) + " " + volume_name + str(volume) , shell=True)
            else:
                if opt not in "--delete-volumes":
                    print "Didn't find volume " + volume_name + str(volume) + " on OpenStack. Going to create it."
                    subprocess.call("openstack volume create --size " + str(volume_size) + " " + volume_name + str(volume) , shell=True)
